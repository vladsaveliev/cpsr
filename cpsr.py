...
def run_cpsr(host_directories, docker_image_version, config_options, sample_id, genome_assembly, cpsr_version, basic, docker_user_id=None):
   ...
   if not input_vcf_docker == 'None':
      ...
      pcgr_vcfanno_command = str(docker_command_run2) + "pcgr_vcfanno.py --num_processes "  + str(config_options['other']['n_vcfanno_proc']) + " --dbnsfp --clinvar --cancer_hotspots --uniprot --pcgr_onco_xref --gwas " + str(vep_vcf) + ".gz " + str(vep_vcfanno_vcf) + " " + os.path.join(data_dir, "data", str(genome_assembly)) + docker_command_run_end
